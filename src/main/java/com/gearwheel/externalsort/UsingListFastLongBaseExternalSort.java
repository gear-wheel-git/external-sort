...
public final class UsingListFastLongBaseExternalSort<Head> implements AutoCloseable {
    ...
    public UsingListFastLongBaseExternalSort(File workdir,
        ...
        log.debug("may use {} at least ", ExternalSortUtils.format(memorySize()));
    }
    ...
    private Map<Head, Integer> createHeadMap(Collection<Head> columnNames) {
        Map<Head, Integer> headIdxMap = new HashMap<>(columnNames.size());
        int i = 0;
        for (Head columnName : columnNames) {
     ...
     */
    public void appendLine(Map<Head, Long> head2RowMap) {
        ...
        for (Map.Entry<Head, Integer> headName2Idx : head.entrySet()) {
     ...
     */
    public void append(Map<Head, List<Long>> head2RowListMap) {
        if (head2RowListMap == null || head2RowListMap.isEmpty()) {
            return;
        }
        int size = 0;
        List<Long> list;
        for (Map.Entry<Head, List<Long>> headName2RowListEntry : head2RowListMap.entrySet()) {
